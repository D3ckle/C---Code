...
int main(int argc, char *argv[])
{
    ...
    pthread_cond_t childCondition; 
    pthread_cond_t parentCondition;
    pthread_cond_init(&childCondition, NULL);
    pthread_cond_init(&parentCondition, NULL);

    arg.mutex = mtx;
    arg.cond_result = childCondition;
    arg.cond_guess = parentCondition;

    pthread_create(&child, NULL, thread_c, &arg);
    pthread_create(&parent, NULL, thread_p, &arg);

    pthread_join(child, NULL);
    pthread_join(parent, NULL);

    pthread_mutex_destroy(&mtx);
    pthread_cond_destroy(&childCondition);
    pthread_cond_destroy(&parentCondition);
