...
void * printer_main(void * arg)
{
    ...
    // the job takes.
    // 4. Use macro print_job() to print. The macro simulates the fact that different print jobs take different
...
int main(int argc, char *argv[])
{
    ...
    pthread_t thread[num_printers]; //have a thread for each printer
    //run each thread
    for (int i = 0; i < num_printers; i++){
        pthread_create(&thread[i], NULL, printer_main, &printers[i]);
    }
