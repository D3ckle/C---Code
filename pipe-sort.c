...
void pipe_sort(int seed, int n, int print_sorted, int num_printed){
    ...
    if (val == 0){  //  child 2:   
        ...
        int temp = write(pd2[1], &b, sizeof(b));
        if (temp == -1){
            perror("write()");
        }
        //      close file descriptor(s) and exit
    ...
    int temp1 = read(pd1[0], &a, sizeof(a));
    //      Results from child 2 are saved in b. 
    int temp2 = read(pd2[0], &b, sizeof(b));
