...
void *producer(void *t) 
{
    ...
    while(!done)
    {
        ...
        while(pdata->n !=0){ // no mroe dice outcomes needed to be made
        for(int i = 1; i < NUM_CONSUMERS+1; i++)
            pthread_cond_wait(&pdata->cond[i], &pdata->mutex); // wait for items to be consumed 
        }
        pdata->n--;
        pdata->data = v;
        pdata->ready = 1;
        pthread_cond_signal(&pdata->cond[0]); // item has been created, signal to eat
...
void * consumer(void *t) 
{
    ...
    while (! done) 
	{
		...
		//Fill in code below
        pthread_mutex_lock(&pdata->mutex);
        while(pdata->ready == 0){ // wait since no data is ready
        ...
        pthread_cond_signal(&pdata->cond[0]); // signal to make more
        pthread_mutex_unlock(&pdata->mutex);
    }
    pthread_exit(NULL);
}
