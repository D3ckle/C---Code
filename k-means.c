...
void* thread_main(void* thread_arg) {
    ...
    double du = clusters(a, labels, m, k, 1, rounds);
    p_squared_intra_cluster_dist = &du;
...
int main(int argc, char *argv[])
{
    ...
    double best_d = clusters(a, labels, m, k, n_threads, rounds);
    //double best_d = squared_intra_cluster_dist(centroids, k, a, n, labels);
    point_t *best_centroids = *centroids; //?
