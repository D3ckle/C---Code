...
node *delete_node(node *head, int v) {
    // TODO
    //head = pointer to the head node, starts as the first node in the linked list
    //int v = the value that is to be deleted

    node *prevn = head;

    //if v is the first element
    if (head->v == v){
        prevn = head->next; //skip over the first one and the second is the new head node
        free(head);
        return prevn; //we found the value to delete, we deleted it by removing access, and finish
    }
  node *currentn = NULL;
    //if v is in the body of the list
    ...
    prevn->next = currentn->next; //change previous head to connect to head after 'v' to delete it
    free(currentn);
    return head;
...
node *reverse_list(node *head) {
    // TODO
    if (head == NULL){error_message(ERR_NOREVERSE);}
