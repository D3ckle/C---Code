...
{
	//m = number of players
	//n = number of properties and # of money gained from passing go
	// p[] = array of players
	// prop[] = array of properties, from 0 to n-1
	for(int i = 0; i < m; i++) //do for each player, i
	{
		int steps = rand() % 6 + 1 + rand() % 6 + 1; //how many steps a player after rolling the die
		//fill in the code below
		//moving to the valid spot
		p[i].loc += steps; //move to the new location according to the die roll

		if (p[i].loc >= n) //loop around to the 'go' spot
		{
			p[i].loc = p[i].loc %n;
			p[i].balance += n; //passes go so gain the money
		}

		if (prop[p[i].loc].owner_id == -1) //landing on unbought properties
		{
			prop[p[i].loc].owner_id = p[i].id; //name the player that landed on this unboght property as the owner
		}
		else //landing on bought properties
		{
