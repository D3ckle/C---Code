...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	while (p->counts[0] == 0 || p->counts[1] == 0 || p->counts[2] == 0) //there is nothing to remove; wait for producers
	{
		pthread_cond_wait(&p->produce_cond, &p->mutex); //there are items now
		pthread_cond_signal(&p->consume_cond); //can consume since there are items now
	}
	//post consuming
	for (int i = 0; i < 3; i++){
		p->counts[i]--;
...
{   
	...
	while(!done)
	{
		...
		//fill in code below
		pthread_mutex_lock(&p->mutex);
		node* product = remove_first(&p->head, &p->tail);
		pthread_mutex_unlock(&p->mutex);
		
		if(product != NULL)
		{
			prepare(product->v);
			add_to_buffer(product->v, product->v, q);
		}
		free(product); //node no longer needed

		if (q->remain == 0){ done = 1;}
